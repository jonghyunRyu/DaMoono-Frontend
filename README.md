...
| 프로필 | 이름 | 주요 역할 및 기여          |
| --- | -- | ------------------- |
| 이미지 | 김예린 | OpenAI + LangChain 기반 텍스트·음성 AI 챗봇 프론트·백엔드 구현, 요금제·구독 상품 응답 로직 설계, 상담사 채팅 UI 및 챗봇 매뉴얼 제작 |
| 이미지 | 류종현 | 마이페이지 및 관리자 페이지 UI 구현, 상담 이력·이용 데이터 시각화, 요금제/구독 정보 조회 UI 개발, 다중 데이터 로딩 렌더링 최적화 및 3D 로딩 UI 적용    |
| 이미지 | 양해강 | 백엔드 프로젝트 및 DB 초기 셋업, 회원가입·인증 API 구현, 전체 API 연동 및 프론트 배포 환경 구성          |
| 이미지 | 이해니 | 상담 요약 페이지(UI) 구현, 유저용 상담 요약 프롬프트(JSON) 설계, 상담사 WebSocket 양방향 통신 구조 설계 및 후속 액션 제안 기능 기획          |
| 이미지 | 이혁준 | 홈·온보딩·성향 분석 페이지 UI 및 컴포넌트 구현, 사용자 성향 분석 로직 설계, 공용 레이아웃(헤더·하단 메뉴) 및 가이드 UI 개발         |
| 이미지 | 유덕 | FAQ 페이지 및 군인·미성년자·대리인 가입 시 구비 서류 안내 UI 구현, 이미지 애니메이션 및 인터랙션 마무리         |
| 이미지 | 최낙운 | 구독 서비스 및 요금제 목록·상세 페이지 UI 구현, 요금제 비교 기능 및 그래프 시각화 개발, 요금제 데이터 백엔드 연동 |
