...
**상담 내용을 요약·정리해주는 통합 요금제 상담 서비스**입니다.
...
| 해결 방식  | AI 추천 + 실시간 상담 병행 + 상담 요약 제공      |
...
| -------------------------------------------- | ----- |
| 이미지 / GIF / 시연 영상                            |       |
| **1. 🔐 소셜 로그인 & 인증 시스템 (OAuth + 인증 상태 관리)** |       |

* Kakao OAuth 기반 소셜 로그인으로 초기 진입 장벽 최소화
* 로그인 이후 사용자 인증 상태에 따라 접근 가능한 화면 제어
* 인증 정보는 보안성을 고려해 쿠키 기반으로 관리
* 전역 상태 관리로 새로고침 및 페이지 이동 시 로그인 상태 유지
* 인증 실패 또는 만료 시 로그인 페이지로 안전하게 리다이렉트 |

| 이미지 / GIF / 시연 영상 |
**2. 🧠 AI 챗봇 요금제 추천 (텍스트 / 음성)**

* OpenAI 기반 챗봇과의 자연어 대화를 통해 사용자 맞춤 요금제 추천
* LangChain Conversation Chain을 활용해
  **의도 분석 → 프롬프트 구성 → 요금제 후보 도출 → 카드형 응답 렌더링** 흐름으로 처리
* 텍스트 입력과 Web Speech API 기반 **음성 입력 모두 지원**
* 성향 테스트 결과를 프롬프트 컨텍스트에 포함하여 추천 정확도 향상
* 대화 중 **정확 모드 / 자연 모드 전환** 가능
* 챗봇 이탈 후 재접속 시 Conversation Memory 기반 **대화 이어서 진행**
* AI 응답을 JSON Schema 형태로 정의해 **AI 로직과 UI 구조 분리** |

| 이미지 / GIF |
**3. 💬 실시간 1:1 상담 시스템 (WebSocket)**

* AI 상담으로 해결되지 않는 경우를 대비한 실시간 상담사 연결 기능
* Socket.IO 기반 WebSocket 통신으로 사용자–상담사 **1:1 Room 매칭**
* 상담 요청 → 대기 → 연결 → 종료 단계의 **세션 상태 머신 관리**
* 상담 종료 시 Room 및 소켓 정리로 안정적인 연결 관리
* 상담사 전용 관리자 모드에서 대기·진행 중 상담 세션 관리 |

| 이미지 / GIF |
**4. 📊 요금제 탐색 및 비교 시각화**

* 통신망(LTE/5G), 가격, 데이터, 혜택 조건 기반 요금제 탐색
* 요금제 공통 데이터 모델 정의로 필터·정렬 로직 단순화
* Chart.js를 활용해 **데이터량 · 요금 · 혜택 비교 시각화**
* Flip Card UI를 통해 핵심 정보와 상세 정보 단계적 노출
* Framer Motion으로 비교 모드 전환 및 카드 인터랙션 애니메이션 구현 |

| 이미지 / GIF |
**5. 🎭 성향 테스트 기반 요금제 추천**

* 데이터 사용량, 통화 빈도, 가격 민감도 등 **6개 카테고리 질문 구성**
* 점수 기반 Rule System으로 사용자 성향 분류
* 테스트 결과를 AI 챗봇 프롬프트에 전달해 추천 로직과 연동
* 질문 및 결과를 JSON 구조로 관리해 문항 확장 용이
* 성향 결과 기반 요금제 카드 자동 추천 |

| 이미지 / GIF |
**6. 📱 채팅 모드 분리 구조**

* AI 채팅 / 상담 채팅 / 관리자 모드 / 가이드 모드로 채팅 구조 분리
* AI 채팅: 요금제 추천 및 정보 제공
* 상담 채팅: WebSocket 기반 실시간 1:1 상담
* 관리자 모드: 상담사 전용 세션 관리
...
| 이미지 | 이름 | 실시간 상담 WebSocket 구현 |
| 이미지 | 이름 | 요금제 비교 및 시각화        |
| 이미지 | 이름 | 성향 테스트 로직           |
| 이미지 | 이름 | 프론트엔드 구조 및 상태 관리    |
